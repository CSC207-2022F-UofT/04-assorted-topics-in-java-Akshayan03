...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String id, Drivable p) {
        if (!drivable_map.containsKey(id)) {
            drivable_map.put(id, p);
            return true;
        } else
            return false;
    }
    ...
    public boolean hasFasterThan(int speed) {
        for (Drivable v : drivable_map.values()) {
        ...
        public List<Tradable> getTradable () {
            ...
            for (Object s : drivable_map.values()) {
                if (s instanceof Tradable) {

                    tradelist.add((Tradable) s);
                }
            }
            return tradelist;
        }
